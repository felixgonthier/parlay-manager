...
// Returns { [nflTeam]: opponentLabel } for games on the slate, or null if the
// schedule can't be read — in which case we fall back to showing everyone
// rather than an empty dropdown.
async function getSlateOpponents(season, week) {
  const cacheKey = `schedule:${season}:${week}:v2`;
  ...
  for (const g of games) {
    ...
    const day = SLATE_DAYS[new Date(`${g.date}T00:00:00Z`).getUTCDay()];
    if (day === undefined) continue;
    map[g.home] = `vs ${g.away}${day}`;
    map[g.away] = `@ ${g.home}${day}`;
  }
...
export async function getLeagueTeams(state) {
  const [users, rosters, players, slate] = await Promise.all([
    j(`/league/${LEAGUE_ID}/users`),
    j(`/league/${LEAGUE_ID}/rosters`),
    getPlayers(),
    state ? getSlateOpponents(state.season, state.week) : null,
  ]);
